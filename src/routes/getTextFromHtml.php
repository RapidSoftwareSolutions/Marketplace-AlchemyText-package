...
$app->post('/api/AlchemyText/getTextFromHtml', function ($request, $response, $args) {
    ...
    if(!empty($post_data['args']['html'])) {
        ...
        $body['html'] = $file;
        $query_str = 'https://gateway-a.watsonplatform.net/calls/url/HTMLGetText?apikey='.$post_data['args']['apiKey'];
    }
    ...
    if(!empty($post_data['args']['extractLinks'])) {
        $body['extractLinks'] = $post_data['args']['extractLinks'];
    }
    if(isset($post_data['args']['useMetadata']) && $post_data['args']['useMetadata']==0) {
        $body['useMetadata'] = $post_data['args']['useMetadata'];
    }
    if(!empty($post_data['args']['cquery'])) {
        $body['cquery'] = $post_data['args']['cquery'];
    }
    if(!empty($post_data['args']['xpath'])) {
        $body['xpath'] = $post_data['args']['xpath'];
    }
    if(!empty($post_data['args']['sourceText'])) {
